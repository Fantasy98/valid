...
train_dl = DataLoader(torch.load(train_path+"/train1.pt"),batch_size=1,shuffle=True
                                                        ,num_workers=0,pin_memory=True)
valid_dl = DataLoader(torch.load(valid_path+"/validation1.pt"),batch_size=1,shuffle=True
                                                        ,num_workers=0,pin_memory=True)
...
torch.save(model,"models/23-1-3/epoch{}.pt".format(EPOCH))
...
plt.legend()
plt.grid()
plt.savefig("/storage3/yuning/thesis/fig/23-1-3/loss_{}".format(EPOCH))

prediction  = pre.cpu().squeeze()
target = y.detach().float().cpu().squeeze()
from utils.plots import Plot_2D_snapshots
Plot_2D_snapshots(prediction,"/storage3/yuning/thesis/fig/23-1-3/pred_{}".format(EPOCH))
Plot_2D_snapshots(target,"/storage3/yuning/thesis/fig/23-1-3/test_{}".format(EPOCH))
